...
//
//     tracing::info!("Starting Chronicle with single database architecture");
//
//     let mut tasks = vec![ServerTask::new(server_config).boxed()];
//
//     for indexer_config in indexer_configs {
//         match IndexerTask::new(indexer_config).await {
//             Ok(task) => tasks.push(task.boxed()),
//             Err(e) => {
//                 tracing::error!("Failed to create indexer task: {}", e);
//                 continue;
//             }
//         }
//     }
//
//     if tasks.len() <= 1 {
//         tracing::warn!("No indexer tasks were successfully created");
//     }
//
